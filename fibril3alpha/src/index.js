...
// Track previous sustain state for release detection
let prevSustainState = false;
...
function clockTick() {
  ...
  const sustainReleased = prevSustainState && !simulatedInputs.sustain;
  prevSustainState = simulatedInputs.sustain;

  // Check if all ranks have zero state (no active keys)
  const allRanksZero = simulatedInputs.ranks.every(rank =>
    rank.every(btn => btn === 0)
  );

  // Trigger algorithm if:
  // 1. Any rank changed, OR
  // 2. Sustain was just released and all ranks are zero
